...
from ttkbootstrap import constants as cts
...
class FileSearchEngine(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.pack(fill=cts.BOTH, expand=cts.YES)
        ...
        self.option_lf.pack(fill=cts.X, expand=cts.YES, anchor=cts.N)
        ...
        self.progressbar = ttk.Progressbar(
            master=self, mode=cts.INDETERMINATE, bootstyle=(cts.STRIPED, cts.SUCCESS)
        )
        self.progressbar.pack(fill=cts.X, expand=cts.YES)

    def create_path_row(self):
        """Add path row to labelframe"""
        path_row = ttk.Frame(self.option_lf)
        path_row.pack(fill=cts.X, expand=cts.YES)
        path_lbl = ttk.Label(path_row, text="Path", width=8)
        path_lbl.pack(side=cts.LEFT, padx=(15, 0))
        path_ent = ttk.Entry(path_row, textvariable=self.path_var)
        path_ent.pack(side=cts.LEFT, fill=cts.X, expand=cts.YES, padx=5)
        browse_btn = ttk.Button(
            master=path_row, text="Browse", command=self.on_browse, width=8
        )
        browse_btn.pack(side=cts.LEFT, padx=5)

    def create_term_row(self):
        """Add term row to labelframe"""
        term_row = ttk.Frame(self.option_lf)
        term_row.pack(fill=cts.X, expand=cts.YES, pady=15)
        term_lbl = ttk.Label(term_row, text="Term", width=8)
        term_lbl.pack(side=cts.LEFT, padx=(15, 0))
        term_ent = ttk.Entry(term_row, textvariable=self.term_var)
        term_ent.pack(side=cts.LEFT, fill=cts.X, expand=cts.YES, padx=5)
        search_btn = ttk.Button(
            master=term_row,
            text="Search",
            command=self.on_search,
            bootstyle=cts.OUTLINE,
            width=8,
        )
        search_btn.pack(side=cts.LEFT, padx=5)

    def create_type_row(self):
        """Add type row to labelframe"""
        type_row = ttk.Frame(self.option_lf)
        type_row.pack(fill=cts.X, expand=cts.YES)
        type_lbl = ttk.Label(type_row, text="Type", width=8)
        type_lbl.pack(side=cts.LEFT, padx=(15, 0))

        contains_opt = ttk.Radiobutton(
            master=type_row, text="Contains", variable=self.type_var, value="contains"
        )
        contains_opt.pack(side=cts.LEFT)
        ...
        startswith_opt.pack(side=cts.LEFT, padx=15)

        endswith_opt = ttk.Radiobutton(
            master=type_row, text="EndsWith", variable=self.type_var, value="endswith"
        )
        endswith_opt.pack(side=cts.LEFT)
        endswith_opt.invoke()

    def create_results_view(self):
        """Add result treeview to labelframe"""
        self.resultview = ttk.Treeview(
            master=self, bootstyle=cts.INFO, columns=[0, 1, 2, 3, 4], show=cts.HEADINGS
        )
        self.resultview.pack(fill=cts.BOTH, expand=cts.YES, pady=10)

        # setup columns and use `scale_size` to adjust for resolution
        self.resultview.heading(0, text="Name", anchor=cts.W)
        self.resultview.heading(1, text="Modified", anchor=cts.W)
        self.resultview.heading(2, text="Type", anchor=cts.E)
        self.resultview.heading(3, text="Size", anchor=cts.E)
        self.resultview.heading(4, text="Path", anchor=cts.W)
        self.resultview.column(
            column=0, anchor=cts.W, width=utility.scale_size(self, 125), stretch=False
        )
        self.resultview.column(
            column=1, anchor=cts.W, width=utility.scale_size(self, 140), stretch=False
        )
        self.resultview.column(
            column=2, anchor=cts.E, width=utility.scale_size(self, 50), stretch=False
        )
        self.resultview.column(
            column=3, anchor=cts.E, width=utility.scale_size(self, 50), stretch=False
        )
        self.resultview.column(
            column=4, anchor=cts.W, width=utility.scale_size(self, 300)
        )
    ...
    def insert_row(self, file, iid):
        ...
        try:
            ...
            _path = file.as_posix()
            iid = self.resultview.insert(
                parent="", index=cts.END, values=(_name, _modified, _type, _size, _path)
            )
